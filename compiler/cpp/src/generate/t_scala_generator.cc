...
class t_scala_generator : public t_oop_generator {
 public:
    ...
    : t_oop_generator(program)
  {
    ...
    out_dir_base_ = (bean_style_ ? "gen-scalabean" : "gen-scala");
  }
  ...
  void generate_scala_struct_definition(std::ofstream& out, t_struct* tstruct, bool is_xception=false, bool in_class=false, bool is_result=false);
  void generate_scala_struct_equality(std::ofstream& out, t_struct* tstruct);
  void generate_scala_struct_compare_to(std::ofstream& out, t_struct* tstruct);
  void generate_scala_struct_reader(std::ofstream& out, t_struct* tstruct);
  void generate_scala_validator(std::ofstream& out, t_struct* tstruct);
  void generate_scala_struct_result_writer(std::ofstream& out, t_struct* tstruct);
  void generate_scala_struct_writer(std::ofstream& out, t_struct* tstruct);
  void generate_scala_struct_tostring(std::ofstream& out, t_struct* tstruct);
  void generate_scala_meta_data_map(std::ofstream& out, t_struct* tstruct);
  ...
  void generate_scala_bean_boilerplate(std::ofstream& out, t_struct* tstruct);
  ...
  void generate_scala_union(t_struct* tstruct);
...
string t_scala_generator::scala_type_imports() {
  ...
  return
    string() +
    hash_builder +
    "import scala.util.List;\n" +
    "import scala.util.ArrayList;\n" +
    "import scala.util.Map;\n" +
    "import scala.util.HashMap;\n" +
    "import scala.util.EnumMap;\n" +
    "import scala.util.Set;\n" +
    "import scala.util.HashSet;\n" +
    "import scala.util.EnumSet;\n" +
    "import scala.util.Collections;\n" +
    "import scala.util.BitSet;\n" +
...
/**
 * Generates a typedef. This is not done in Scala, since it does
...
/**
 * Scala union definition.
 *
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_union(t_struct* tstruct) {
  // Make output file
  string f_struct_name = package_dir_+"/"+(tstruct->get_name())+".scala";
  ofstream f_struct;
  f_struct.open(f_struct_name.c_str());
  ...
  generate_scala_doc(f_struct, tstruct);
  ...
  generate_scala_meta_data_map(f_struct, tstruct);
...
void t_scala_generator::generate_union_constructor(ofstream& out, t_struct* tstruct) {
...
void t_scala_generator::generate_union_getters_and_setters(ofstream& out, t_struct* tstruct) {
  const vector<t_field*>& members = tstruct->get_members();
  vector<t_field*>::const_iterator m_iter;
...
void t_scala_generator::generate_union_abstract_methods(ofstream& out, t_struct* tstruct) {
...
void t_scala_generator::generate_check_type(ofstream& out, t_struct* tstruct) {
  indent(out) << "@Override" << endl;
...
void t_scala_generator::generate_read_value(ofstream& out, t_struct* tstruct) {
  indent(out) << "@Override" << endl;
...
void t_scala_generator::generate_write_value(ofstream& out, t_struct* tstruct) {
  indent(out) << "@Override" << endl;
...
void t_scala_generator::generate_get_field_desc(ofstream& out, t_struct* tstruct) {
  indent(out) << "@Override" << endl;
...
void t_scala_generator::generate_get_struct_desc(ofstream& out, t_struct* tstruct) {
  indent(out) << "@Override" << endl;
...
void t_scala_generator::generate_union_comparisons(ofstream& out, t_struct* tstruct) {
  // equality
...
void t_scala_generator::generate_union_hashcode(ofstream& out, t_struct* tstruct) {
  if (gen_hash_code_) {
    indent(out) << "@Override" << endl;
...
void t_scala_generator::generate_scala_struct_definition(ofstream &out,
  ...
  out << "implements TBase<" << tstruct->get_name() << "._Fields>, scala.io.Serializable, Cloneable";
  ...
  // Members are public for -scala, private for -scalabean
  const vector<t_field*>& members = tstruct->get_members();
  vector<t_field*>::const_iterator m_iter;
  ...
  generate_scala_meta_data_map(out, tstruct);
  ...
  generate_scala_bean_boilerplate(out, tstruct);
  generate_generic_field_getters_setters(out, tstruct);
  generate_generic_isset_method(out, tstruct);

  generate_scala_struct_equality(out, tstruct);
  if (is_comparable(tstruct)) {
    generate_scala_struct_compare_to(out, tstruct);
  }

  generate_scala_struct_reader(out, tstruct);
  if (is_result) {
    generate_scala_struct_result_writer(out, tstruct);
  } else {
    generate_scala_struct_writer(out, tstruct);
  }
  generate_scala_struct_tostring(out, tstruct);
  generate_scala_validator(out, tstruct);
  scope_down(out);
  out << endl;
}
 ...
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_struct_equality(ofstream& out,
                                                     t_struct* tstruct) {
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    if (t->is_base_type() && ((t_base_type*)t)->is_binary()) {
      unequal = "!scala.util.Arrays.equals(this." + name + ", that." + name + ")";
    } else if (can_be_null) {
...
void t_scala_generator::generate_scala_struct_compare_to(ofstream& out, t_struct* tstruct) {
  indent(out) << "public int compareTo(" << type_name(tstruct) << " other) {" << endl;
  indent_up();
 ...
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_struct_reader(ofstream& out,
                                                   t_struct* tstruct) {
  out <<
...
// generates scala method to perform various checks
// (e.g. check that all required fields are set)
void t_scala_generator::generate_scala_validator(ofstream& out,
                                                   t_struct* tstruct){
 ...
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_struct_writer(ofstream& out,
                                                   t_struct* tstruct) {
  out <<
 ...
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_struct_result_writer(ofstream& out,
                                                          t_struct* tstruct) {
  out <<
    indent() << "public void write(TProtocol oprot) throws TException {" << endl;
  indent_up();
...
void t_scala_generator::generate_reflection_getters(ostringstream& out, t_type* type, string field_name, string cap_name) {
...
void t_scala_generator::generate_reflection_setters(ostringstream& out, t_type* type, string field_name, string cap_name) {
  indent(out) << "case " << constant_name(field_name) << ":" << endl;
  indent_up();
...
void t_scala_generator::generate_generic_field_getters_setters(std::ofstream& out, t_struct* tstruct) {
...
void t_scala_generator::generate_generic_isset_method(std::ofstream& out, t_struct* tstruct){
  const vector<t_field*>& fields = tstruct->get_members();
  vector<t_field*>::const_iterator f_iter;
...
/**
 * Generates a set of Scala Bean boilerplate functions (setters, getters, etc.)
 * for the given struct.
 *
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_bean_boilerplate(ofstream& out,
                                                      t_struct* tstruct) {
  const vector<t_field*>& fields = tstruct->get_members();
  vector<t_field*>::const_iterator f_iter;
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    if (type->is_set() || type->is_list()) {
      ...
      indent(out) << "public scala.util.Iterator<" <<
 ...
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_struct_tostring(ofstream& out,
                                                     t_struct* tstruct) {
  out << indent() << "@Override" << endl <<
 ...
 * @param tstruct The struct definition
 */
void t_scala_generator::generate_scala_meta_data_map(ofstream& out,
                                                   t_struct* tstruct) {
  const vector<t_field*>& fields = tstruct->get_members();
  vector<t_field*>::const_iterator f_iter;
...
void t_scala_generator::generate_field_value_meta_data(std::ofstream& out, t_type* type){
  out << endl;
  indent_up();
  indent_up();
  if (type->is_struct()){
    ...
    indent(out) << "new FieldValueMetaData(" << get_scala_type_string(type);
  }
  out << ")";
  indent_down();
  indent_down();
...
void t_scala_generator::generate_service_interface(t_service* tservice) {
  ...
  generate_scala_doc(f_service_, tservice);
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    generate_scala_doc(f_service_, *f_iter);
...
void t_scala_generator::generate_service_helpers(t_service* tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    generate_scala_struct_definition(f_service_, ts, false, true);
 ...
 */
void t_scala_generator::generate_function_helpers(t_function* tfunction) {
  ...
  generate_scala_struct_definition(f_service_, &result, false, true, true);
}
 ...
 */
void t_scala_generator::generate_process_function(t_service* tservice,
 ...
 */
void t_scala_generator::generate_deserialize_field(ofstream& out,
                                                  t_field* tfield,
                                                  string prefix) {
  ...
  if (type->is_struct() || type->is_xception()) {
    ...
    switch (tbase) {
      ...
      case t_base_type::TYPE_DOUBLE:
        ...
        break;
      default:
        throw "compiler error: no Scala name for base type " + t_base_type::t_base_name(tbase);
    }
    out << endl;
  } else if (type->is_enum()) {
 ...
 */
void t_scala_generator::generate_deserialize_struct(ofstream& out,
                                                   t_struct* tstruct,
                                                   string prefix) {
  out <<
 ...
 */
void t_scala_generator::generate_deserialize_container(ofstream& out,
                                                      t_type* ttype,
                                                      string prefix) {
  scope_up(out);
 ...
 */
void t_scala_generator::generate_deserialize_map_element(ofstream& out,
                                                        t_map* tmap,
                                                        string prefix) {
  string key = tmp("_key");
  string val = tmp("_val");
 ...
 */
void t_scala_generator::generate_deserialize_set_element(ofstream& out,
                                                        t_set* tset,
                                                        string prefix) {
 ...
 */
void t_scala_generator::generate_deserialize_list_element(ofstream& out,
                                                         t_list* tlist,
                                                         string prefix) {
  string elem = tmp("_elem");
 ...
 */
void t_scala_generator::generate_serialize_field(ofstream& out,
                                                t_field* tfield,
                                                string prefix) {
  t_type* type = get_true_type(tfield->get_type());
 ...
 */
void t_scala_generator::generate_serialize_struct(ofstream& out,
                                                 t_struct* tstruct,
                                                 string prefix) {
  out <<
 ...
 */
void t_scala_generator::generate_serialize_container(ofstream& out,
                                                    t_type* ttype,
                                                    string prefix) {
  scope_up(out);
 ...
 */
void t_scala_generator::generate_serialize_map_element(ofstream& out,
                                                      t_map* tmap,
                                                      string iter,
                                                      string map) {
 ...
 */
void t_scala_generator::generate_serialize_set_element(ofstream& out,
                                                      t_set* tset,
                                                      string iter) {
 ...
 */
void t_scala_generator::generate_serialize_list_element(ofstream& out,
                                                       t_list* tlist,
                                                       string iter) {
 ...
 * @param ttype The type
 */
string t_scala_generator::declare_field(t_field* tfield, bool init) {
 ...
 */
string t_scala_generator::function_signature(t_function* tfunction,
                                            string prefix) {
 ...
 */
string t_scala_generator::argument_list(t_struct* tstruct) {
  string result = "";
 ...
 */
string t_scala_generator::type_to_enum(t_type* type) {
  type = get_true_type(type);
...
void t_scala_generator::generate_deep_copy_container(ofstream &out, std::string source_name_p1, std::string source_name_p2,
...
void t_scala_generator::generate_deep_copy_non_container(ofstream& out, std::string source_name, std::string dest_name, t_type* type) {
...
std::string t_scala_generator::generate_isset_check(t_field* field) {
  return generate_isset_check(field->get_name());
}

std::string t_scala_generator::isset_field_id(t_field* field) {
  return "__" + upcase_string(field->get_name() + "_isset_id");
}

std::string t_scala_generator::generate_isset_check(std::string field_name) {
  return "is" + get_cap_name("set") + get_cap_name(field_name) + "()";
}

void t_scala_generator::generate_isset_set(ofstream& out, t_field* field) {
  if (!type_can_be_null(field->get_type())) {
...
std::string t_scala_generator::get_enum_class_name(t_type* type) {
  ...
  if (program != NULL && program != program_) {
    package = program->get_namespace("scala") + ".";
  }
  return package + type->get_name();
}

void t_scala_generator::generate_struct_desc(ofstream& out, t_struct* tstruct) {
  indent(out) <<
    "private static final TStruct STRUCT_DESC = new TStruct(\"" << tstruct->get_name() << "\");" << endl;
}

void t_scala_generator::generate_field_descs(ofstream& out, t_struct* tstruct) {
  const vector<t_field*>& members = tstruct->get_members();
  vector<t_field*>::const_iterator m_iter;
...
void t_scala_generator::generate_field_name_constants(ofstream& out, t_struct* tstruct) {
...
bool t_scala_generator::is_comparable(t_struct* tstruct) {
  const vector<t_field*>& members = tstruct->get_members();
  vector<t_field*>::const_iterator m_iter;
...
bool t_scala_generator::is_comparable(t_type* type) {
  if (type->is_container()) {
    if (type->is_list()) {
...
bool t_scala_generator::has_bit_vector(t_struct* tstruct) {
  const vector<t_field*>& members = tstruct->get_members();
  vector<t_field*>::const_iterator m_iter;
